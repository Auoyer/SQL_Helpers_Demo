...
namespace Utils
{
    ...
    public static class CookieHelper
    {
        ...
        public static void Add(string strCookieName, string strValue, int iExpires = 0)
        {
        ...
        public static void Add(string strCookieName, NameValueCollection KeyValue, int iExpires = 0)
        {
            HttpCookie objCookie = new HttpCookie(strCookieName.Trim());
        ...
        public static string Edit(string strCookieName, string strKeyName, string KeyValue, int iExpires=0)
        {
            if (HttpContext.Current.Request.Cookies[strCookieName] == null)
                return null;
            else
        ...
        /// <param name="iExpires">COOKIE对象有效时间（秒数），1表示永久有效，0和负数都表示不设有效时间，大于等于2表示具体有效秒数，31536000秒=1年=(60*60*24*365)。注意：虽是修改功能，实则重建覆盖，所以时间也要重设，因为没办法获得旧的有效期</param>  
        /// <returns>如果对象本就不存在，则返回字符串null，如果操作成功返回字符串"success"。</returns>  
        public static string DelKey(string strCookieName, string strKeyName, int iExpires = 0)
        {
            if (HttpContext.Current.Request.Cookies[strCookieName] == null)
            {
                return null;
